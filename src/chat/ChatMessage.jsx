...
export function MessageItem(props) {
  const { content, timestamp, role, latex, index } = props
  const { removeMessage, openOverLeaf } = useGlobal()
  return (
      ...
      <div className={classnames(styles.item_content, styles[`item_${role}`])}>
        <div className={styles.item_inner}>
          <div className={styles.item_tool}>
            <div className={styles.item_date}>{dateFormat(timestamp)}</div>
            <div className={styles.item_bar}>
              <Tooltip text="Remove Messages">
                <Icon className={styles.icon} type="trash" onClick={() => removeMessage(index)} />
              </Tooltip>
              ...
              <React.Fragment>
                {latex ? <Tooltip text="Export to Overleaf">
                  <Icon className={styles.icon} type="export" onClick={() => openOverLeaf(content)} />
                </Tooltip> : <></>}
...
export function MessageBar() {
  const { sendMessage, setMessage, is, options, setIs, typeingMessage, clearTypeing, stopResonse, inputTextKey } = useGlobal()
    ...
    <div className={styles.bar}>
      ...
      <div className={styles.bar_inner}>
        <div className={styles.bar_type}>
          <Textarea transparent={true} rows="3" key={inputTextKey} value={typeingMessage?.content || ''} onFocus={() => setIs({ inputing: true })} onBlur={() => setIs({ inputing: false })} placeholder="Enter something...." onChange={setMessage} />
        </div>
      ...
      <React.Fragment>
        {
          messages.length ? <div className={styles.container}>
            {messages.map((item, index) => <MessageItem key={index} {...item} index={index} />)}
            {message?.error && <Error />}
          </div> : <ChatHelp />
        }
      </React.Fragment>
